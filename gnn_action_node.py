...
# pretrained
initial_path = "./MODEL/2GCN 128_128 5 class.ckpt"
checkpoint_path = "./MODEL/2GCN 128_128 5 class.ckpt"
model_file = './MODEL/2GCN 64_128 1FC 128 5 class_20000.h5'
model_load_file = './MODEL/GCN_action_node_siamese_model3 3 class pretrained.h5'
...
# 自定义的图卷积层，处理节点特征、边特征、邻接链表和全局特征
class CustomGraphConvLayer(GCNConv):
    def __init__(self, channels, **kwargs):
        super().__init__(channels, **kwargs)

    def call(self, inputs):
        X, A, E, G = inputs
        output = super().call([X, A])
        if E is not None:
            edge_features = tf.matmul(E, self.kernel)
            output += tf.matmul(A, edge_features)
        if G is not None:
            global_features = tf.matmul(G, self.kernel)
