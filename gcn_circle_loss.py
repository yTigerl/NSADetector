...
# pretrained
initial_path = "./MODEL/2GAT 32_64 1FC 128 4 class circle siamese.ckpt"
checkpoint_path = "./MODEL/2GAT 32_64 1FC 128 4 class circle siamese.ckpt"
model_file = './MODEL/2GCN 64_64 1FC 128  data_4_class_20000 circle cosine siamese.h5'
model_load_file = './MODEL/2GCN 128_256 1FC 256 class_20000.h5'
# initial_path = "./MODEL/2GCN 128_128 1FC 128 5 class.ckpt"
# checkpoint_path = "./MODEL/2GCN 128_128 1FC 128 5 class.ckpt"
# model_file = './MODEL/2GCN 128_128 1FC 128 data_5_class_20000_30000 MBS.h5'
# model_load_file = './MODEL/2GCN 128_128 1FC 128 class_20000.h5'
# model_load_file = './MODEL/2GCN 128_128 1FC 128 data_5_class_20000_20100 LPM .h5'
...
def create_siamese_model(input_shapes, output_units):
    base_model = create_gcn_model(input_shapes, output_units)
    ...
    distance = cosine_similarity_layer()([GCN_out1, GCN_out2])

    return Model(inputs=[X_in1, A_in1, X_in2, A_in2], outputs=distance)

def cosine_similarity_layer():
    return Lambda(lambda tensors: K.sum(
    K.l2_normalize(tensors[0], axis=1) * K.l2_normalize(tensors[1], axis=1),
    axis=1,
    keepdims=True))
...
def circle_loss(y_true, y_pred, margin=0.25, gamma=64):
    y_true = K.cast(y_true, dtype='float32')  # (batch_size, 1)
    y_true = K.squeeze(y_true, axis=-1)
    y_pred = K.squeeze(y_pred, axis=-1)  # (batch_size,)

    # 筛选正对和负对
    mask_pos = tf.equal(y_true, 1.0)
    mask_neg = tf.equal(y_true, 0.0)

    sp = tf.boolean_mask(y_pred, mask_pos)  # 正样本对的相似度
    sn = tf.boolean_mask(y_pred, mask_neg)  # 负样本对的相似度

    # 权重项（由相似度决定）
    ap = K.relu(-sp + 1 + margin)  # 正样本对的权重因子
    an = K.relu(sn + margin)  # 负样本对的权重因子

    # logit
    logit_p = - gamma * ap * (sp - 1 + margin)
    logit_n = gamma * an * (sn + margin)
    # print(logit_p)
    # print(logit_n)

    # log-sum-exp 聚合
    loss_p = K.sum(K.exp(logit_p))
    loss_n = K.sum(K.exp(logit_n))
    # print(logit_p)
    # print(logit_n)

    # 最终损失
    loss = K.log(1 + loss_p * loss_n)
    # print(loss)
    return loss
...
model.compile(optimizer=sgd, loss=circle_loss, metrics=['accuracy'])
